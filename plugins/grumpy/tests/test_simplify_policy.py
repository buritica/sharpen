...
class TempRepo:
    ...
    def git(self, *args):
        return subprocess.run(
            ["git", "-C", self.dir] + list(args), check=True, capture_output=True, text=True
        ).stdout
...
class ConfigTests(unittest.TestCase):
    ...
    def test_explicit_missing_path_is_error(self):
        with self.assertRaises(sp.ConfigError):
            sp.load_config(worktree=HERE, explicit_path=os.path.join(self.tmp, "nope.json"))
    ...
    def test_language_override_by_extension(self):
        p = self._write({"languages": {"rs": {"thresholds": {"cyclomatic": 30},
    ...
    def test_readme_embeds_example_config_verbatim(self):
        ...
        with open(README) as fh:
            readme = fh.read()
        self.assertIn(example, readme, "README's config example drifted from the fixture")
...
class ClassificationTests(unittest.TestCase):
    ...
    def test_is_excluded_generated_header(self):
        ...
        self.assertEqual(sp.excluded_reason("src/gen.rs", cfg(), text), "generated-header")
    ...
    def test_file_kind_test_dirs_and_names(self):
        ...
        for p in ("src/main.rs", "src/testing_utils.py", "contest.py", "src/spec_parser.go"):
    ...
    def test_debt_record_glob_and_metric(self):
        ...
        self.assertEqual(sp.debt_record(c, "src/main.rs", "loc_per_file")["reason"], "r1")
...
class ValueMetricTests(unittest.TestCase):
    def cyc(self, base, head, confidence="measured:radon", **kw):
        return judge(metric="cyclomatic", file="src/a.py", symbol="f",
                     base=base, head=head, confidence=confidence, **kw)
    ...
    def test_new_symbol_over_threshold_blocks(self):
        f = self.cyc(None, 25)
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("new", True, "CRIT"))
    ...
    def test_improved(self):
        f = self.cyc(40, 39)
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("improved", False, "NOTE"))
    ...
    def test_regressed_beyond_tolerance_blocks(self):
        f = self.cyc(40, 43)
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("regressed", True, "CRIT"))

    def test_estimated_complexity_never_blocks(self):
        f = self.cyc(40, 43, confidence="estimated")
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("regressed", False, "WARN"))
    ...
    def test_blocking_reason_lists_every_suppression(self):
        f = judge(metric="halstead", file="tests/test_a.py", symbol="t", base=None, head=90,
                  confidence="estimated")
        self.assertEqual(f["blocking_reason"],
                         "advisory metric; test file; estimated, not measured")
    ...
    def test_block_on_estimate_is_configurable(self):
        ...
        f = judge(metric="any_unknown", file="src/a.ts", line=1, introduced=True,
                  confidence="estimated", _cfg=c)
    ...
    def test_advisory_metric_warns_not_blocks(self):
        f = judge(metric="halstead", file="a.py", symbol="f", base=None, head=90,
                  confidence="measured:radon")
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("new", False, "WARN"))
        self.assertIn("advisory", f["note"])

    def test_test_file_never_blocks(self):
        f = judge(metric="cyclomatic", file="tests/test_a.py", symbol="t", base=None, head=30,
                  confidence="measured:radon")
        self.assertEqual((f["status"], f["blocking"], f["kind"]), ("new", False, "test"))

    def test_explicit_kind_wins(self):
        f = judge(metric="cyclomatic", file="src/a.py", symbol="t", base=None, head=30,
                  confidence="measured:radon", kind="test")
        self.assertFalse(f["blocking"])

    def test_debt_record_turns_block_into_excepted(self):
        c = cfg(debt=[{"path": "src/*.py", "reason": "tracked in #9"}])
        f = self.cyc(40, 50, _cfg=c)
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("excepted", False, "WARN"))
    ...
    def test_debt_record_does_not_touch_non_blocking(self):
        ...
        self.assertEqual(self.cyc(40, 43, confidence="estimated", _cfg=c)["status"], "regressed")

    def test_coverage_direction(self):
        cov = lambda b, h, c="measured:coverage": judge(  # noqa: E731
            metric="coverage", file="src/a.py", symbol="f", base=b, head=h, confidence=c)
        ...
        self.assertEqual((cov(100, 95)["status"], cov(100, 95)["blocking"]), ("new", True))
        self.assertEqual((cov(None, 80)["status"], cov(None, 80)["blocking"]), ("new", True))
    ...
    def test_language_override_makes_head_compliant(self):
        ...
        f = judge(metric="cyclomatic", file="src/main.rs", symbol="f", base=None, head=25,
...
class LocMetricTests(unittest.TestCase):
    def loc(self, base, head, path="src/main.rs", **kw):
        return judge(metric="loc_per_file", file=path, base=base, head=head,
                     confidence=sp.LOC_CONFIDENCE, **kw)
    ...
    def test_new_file_strong_warn_tier_does_not_block(self):
        ...
        self.assertEqual((f["status"], f["blocking"], f["severity"], f["tier"]),
                         ("new", False, "WARN", "strong-warn"))
        self.assertEqual(f["blocking_reason"], "below the hard-block tier for a new file")

    def test_new_file_warn_tier_does_not_block(self):
        f = self.loc(None, 600)
        self.assertEqual((f["status"], f["blocking"], f["tier"]), ("new", False, "warn"))
    ...
    def test_crossing_healthy_is_new_non_blocking(self):
        self.assertEqual((self.loc(490, 520)["status"], self.loc(490, 520)["blocking"]), ("new", False))
    ...
    def test_legacy_growth_with_debt_record_is_excepted(self):
        c = cfg(debt=[{"path": "src/main.rs", "metric": "loc_per_file", "reason": "see #12"}])
    ...
    def test_language_tiers_override(self):
        ...
        self.assertEqual(self.loc(None, 1600, path="db/schema.sql", _cfg=c)["status"], "compliant")
...
class CountMetricTests(unittest.TestCase):
    ...
    def test_redundant_estimated_does_not_block(self):
        f = judge(metric="redundant_code", file="src/a.ts", line=12, introduced=True,
                  confidence="estimated")
        self.assertEqual((f["status"], f["blocking"], f["severity"]), ("new", False, "WARN"))
    ...
    def test_introduced_any_in_test_file_still_blocks(self):
        f = judge(metric="any_unknown", file="tests/helpers.test.ts", line=1, introduced=True)
    ...
    def test_test_advisory_is_configurable(self):
        ...
        c = cfg(test_advisory=[])
        f = judge(metric="cyclomatic", file="tests/test_a.py", symbol="t", base=None, head=30,
                  confidence="measured:radon", _cfg=c)
        self.assertTrue(f["blocking"])
...
class JudgeAllTests(unittest.TestCase):
    ...
    def test_passes_with_debt_verdict(self):
        r = sp.judge_all([
            {"metric": "loc_per_file", "file": "src/main.rs", "base": 8471, "head": 8243,
             "confidence": sp.LOC_CONFIDENCE},
            {"metric": "any_unknown", "file": "src/a.ts", "line": 4, "introduced": False},
        ], cfg())
    ...
    def test_error_names_finding_index(self):
        with self.assertRaises(sp.FindingError) as ctx:
            sp.judge_all([{"metric": "cyclomatic", "file": "a", "head": 1},
                          {"metric": "nope", "file": "a", "head": 1}], cfg())
...
class LocMeasurementTests(unittest.TestCase):
    ...
    def test_archivaldo_extraction_passes_with_debt(self):
        ...
        self.assertEqual((files["src/main.rs"]["base"], files["src/main.rs"]["head"]), (8471, 8243))
    ...
    def test_growth_with_debt_record_is_excepted(self):
        self.repo.write(".sharpen/simplify.json", json.dumps({"debt": [
            {"path": "src/main.rs", "reason": "tracked"}]}))
        self.repo.write("src/main.rs", lines(8890, "main"))
    ...
    def test_rename_reads_base_under_old_name(self):
        ...
        self.assertEqual((files["src/app.rs"]["base"], files["src/app.rs"]["status"]),
                         (8471, "improved"))

    def test_committed_only_diff_is_measured(self):
        self.repo.write("src/main.rs", lines(8243, "main"))
        self.repo.commit("shrink")
        self.assertEqual(self.by_file(self.report())["src/main.rs"]["status"], "improved")
    ...
    def test_binary_file_is_skipped(self):
        ...
        self.assertIn({"file": "src/blob.bin", "reason": "binary"}, r["summary"]["excluded"])
...
class CliTests(unittest.TestCase):
    ...
    def test_judge_json_lines(self):
        stdin = "\n".join([
            json.dumps({"metric": "cyclomatic", "file": "a.py", "symbol": "f", "base": 40,
                        "head": 41, "confidence": "measured:radon"}),
            "",
            json.dumps({"metric": "any_unknown", "file": "a.ts", "line": 1, "introduced": True}),
        ])
    ...
    def test_judge_json_array(self):
        stdin = json.dumps([{"metric": "cyclomatic", "file": "a.py", "head": 1,
