...
- `auto-init-gate-cycle.py` (PostToolUse Bash) — starts a `small-medium` cycle on the first `git commit` to a non-default branch, which is what makes enforcement opt-out rather than opt-in. It announces the arming (exit 2 — visible, non-blocking; the commit already ran), because for anyone who never runs `/sdlc:gate` that is the only notice that gates 2–6 now need the skills, or a re-init as `tiny`. A commit that leaves the branch with *no* cycle says so the same way; genuinely routine outcomes stay quiet.
